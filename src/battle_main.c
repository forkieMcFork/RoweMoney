...
const struct TrainerMoney gTrainerMoneyTable[] =
{
    ...
    {TRAINER_CLASS_BUG_CATCHER, 40},
    {TRAINER_CLASS_HIKER, 10},
    {TRAINER_CLASS_YOUNG_COUPLE, 8},
    {TRAINER_CLASS_WINSTRATE, 10},
	{TRAINER_CLASS_PKMN_MASTER, 50},
	{TRAINER_CLASS_CRUSH_KIN, 8},
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
	...
	u8 levelboost = getLevelDifference();
	...
	u8 speciesnumber;
	u8 abilityNum = 0;
	bool8 isHardMockBattle = FALSE;
	bool8 isNormalMockBattle = FALSE;
	u16 hardMockBattles = 0 + VarGet(VAR_HARD_MOCK_BATTLES);
	u16 normalMockBattles = 0 + VarGet(VAR_NORMAL_MOCK_BATTLES);
	bool8 canMega = FALSE;
	u16 randomMove = MOVE_TACKLE;
    struct CompressedPlayerParty CompressedPlayerParty;
    bool8 passwordTrainer = FALSE;

    if(FlagGet(FLAG_PASSWORD_TRAINER)){
        CompressedPlayerParty = CreatePartyFromRamData();
        passwordTrainer = TRUE;
    }
	
	if(PartySize >= 2 && gTrainers[trainerNum].partySize >= 2 && FlagGet(FLAG_DOUBLE_BATTLE_MODE))
		isDoubleBattle = TRUE;
	else if(PartySize >= 2 && gTrainers[trainerNum].partySize >= 2 && gTrainers[trainerNum].doubleBattle)
		isDoubleBattle = TRUE;
	else
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        if(passwordTrainer){
            monsCount = CompressedPlayerParty.partySize;
        }
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            case 0:
            {
				...
				//Changes Pokemon depending on the map
				mapspecies = GetMapRandomPokemon(gTrainers[trainerNum].trainerClass, partyData[i].species);
				
				//Get Trainer Species
				if (partyData[i].lvl == 6){//Random Pokemon with scaled level
					if(GetPlayerPartyMaxLevel() < (LeaderMinLevel + monsCount)){
						newspecies = GetTrainerPokemon(GetRandomFirstStage(SPECIES_NONE), LeaderMinLevel + monsCount);
						pokemonLevel = LeaderMinLevel  + monsCount;
					}
					else{
						newspecies = GetTrainerPokemon(GetRandomFirstStage(SPECIES_NONE), GetPlayerPartyMaxLevel());
						pokemonLevel = GetPlayerPartyMaxLevel();
					}
						
					//Achievement
					isHardMockBattle = TRUE;
				}
				else if (partyData[i].lvl == 5){//Random Pokemon with an strong Team
					newspecies = GetTrainerPokemon(GetRandomFirstStage(SPECIES_NONE), LeaderMinLevel);
					pokemonLevel = LeaderMinLevel;
					
					isNormalMockBattle = TRUE;
				}
				else if (partyData[i].lvl == 3){
					newspecies = GetTrainerPokemon(partyData[i].species, LeaderMinLevel);
					pokemonLevel = LeaderMinLevel;
				}
				else if (partyData[i].lvl == 2){//Random Pokemon with a random level
					newspecies = GetTrainerPokemon(GetRandomFirstStage(SPECIES_NONE), TrainerLevel[i]);
					pokemonLevel = TrainerLevel[i];
				}
				else{
					newspecies = GetTrainerPokemon(mapspecies, TrainerLevel[i]);
					pokemonLevel = TrainerLevel[i];
				}
				...
				if(newspecies == SPECIES_SMEARGLE){
					for (j = 0; j < MAX_MON_MOVES; j++)
					{
						...
						if(randomMove == MOVES_COUNT || gBattleMoves[randomMove].effect == EFFECT_PLACEHOLDER)
							randomMove = MOVE_RETURN;
						
						SetMonData(&party[i], MON_DATA_MOVE1 + j, &randomMove);
					}
				}
				
				//Sets the Pokeball
				if(gTrainers[trainerNum].trainerClass == TRAINER_CLASS_NURSE){
					newball = ITEM_HEAL_BALL;
					SetMonData(&party[i], MON_DATA_POKEBALL, &newball);
				}
				
				if((gTrainers[trainerNum].trainerClass == TRAINER_CLASS_NURSE   || 
				   gTrainers[trainerNum].trainerClass == TRAINER_CLASS_EXPERT) && Random() % 2 == 0)
				   canMega = TRUE;
            ...
            case F_TRAINER_PARTY_CUSTOM_MOVESET:
            {
				...
				if (partyData[i].lvl == 1){
					...
					newspecies = noEvolutionModeCheck(partyData[i].species);
					pokemonLevel = partyData[i].lvl + levelboost;
				}
                ...
                //Mock battle using your own Pokémon
                if (partyData[i].lvl == 6 && GetBoxMonData(&gPokemonStoragePtr->boxes[0][i], MON_DATA_SPECIES, NULL) != SPECIES_NONE){
                    BoxMonToMon(&gPokemonStoragePtr->boxes[0][i], &party[i]);
                }
                else if(partyData[i].lvl == 7 && passwordTrainer){
                    struct Pokemon newMon = CreateMonFromPartyRamData(i);
                    if(GetMonData(&newMon, MON_DATA_SPECIES, NULL) != SPECIES_NONE){
                        party[i] = newMon;
                    }
                }
            ...
            case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM:
            {
				...
				if (partyData[i].isShiny)          
					OTShiny = OT_ID_RANDOM_FORCED_SHINY;
				else
					OTShiny = OT_ID_RANDOM_NO_SHINY;
			
				//Get Trainer Species ------------------------------------------------------------------------------------------
				if (partyData[i].lvl == 1)//trainer class based mons with dynamic level
				{
					newspecies = GetTrainerPokemon(partyData[i].species, TrainerLevel[i]);
					pokemonLevel = TrainerLevel[i];
				}
				else if (partyData[i].lvl == 3)//static mons with a little more difficult level(mostly used for gym trainers)
				{
					newspecies = GetTrainerPokemon(partyData[i].species, LeaderMinLevel);
					pokemonLevel = LeaderMinLevel;
				}
				else if (partyData[i].lvl == 5)//Used for gym leaders
				{
				...
				//Create Mon --------------------------------------------------------------------------------------
				CreateMon(&party[i], newspecies, pokemonLevel, fixedIV, TRUE, personalityValue, OTShiny, 0, formId);

				// Sets the Held Item ---------------------------------------------------------------------------------
                if(numBadges >= 8 && partyData[speciesnumber].postgameheldItem != ITEM_NONE)
					SetMonData(&party[i], MON_DATA_HELD_ITEM, &partyData[speciesnumber].postgameheldItem);	
				else{
					PokemonHeldItem[i] = GetHeldItem(partyData[speciesnumber].heldItem);
					SetMonData(&party[i], MON_DATA_HELD_ITEM, &PokemonHeldItem[i]);	
				}
				...
				if(partyData[speciesnumber].abilityNum < 3)
					SetMonData(&party[i], MON_DATA_ABILITY_NUM, &partyData[speciesnumber].abilityNum);
				...
				//Sets the Pokeball
				SetMonData(&party[i], MON_DATA_POKEBALL, &partyData[speciesnumber].pokeball);
				
				//Sets the exioliteBoost
				if(partyData[i].exioliteBoost)
					exioliteboost = 1;
				
				SetMonData(&party[i], MON_DATA_EXIOLITE_ENABLED, &exioliteboost);
		...
		if(isHardMockBattle){
			FlagSet(FLAG_IS_HARD_MOCK_BATTLE);
			hardMockBattles++;
			VarSet(VAR_HARD_MOCK_BATTLES, hardMockBattles);
		}
		else if(isNormalMockBattle){
			normalMockBattles++;
			VarSet(VAR_NORMAL_MOCK_BATTLES, normalMockBattles);
		}
...
static void BattleStartClearSetData(void)
{
	...
	//Achievement
	if(FlagGet(FLAG_IS_HARD_MOCK_BATTLE) && !FlagGet(FLAG_WON_METRONOME_HARD_BATTLE)){
		if(CheckforMetronomeBattle())
			FlagSet(FLAG_WON_METRONOME_HARD_BATTLE);
	}

	FlagClear(FLAG_IS_HARD_MOCK_BATTLE);
	FlagClear(FLAG_SMART_AI);
	//FlagClear(FLAG_TOTEM_BATTLE);
    //VarSet(VAR_NUM_WILD_POKEMON_STAT_BOOST, 0);
...
u32 GetBattlerTotalSpeedStat(u8 battlerId)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        if (ability == ABILITY_SWIFT_SWIM       && gBattleWeather & WEATHER_RAIN_ANY)
            ...
            speed *= 2;
		else if (ability == ABILITY_FLOWER_GIFT && gBattleWeather & WEATHER_SUN_ANY)
            speed *= 1.5;
    }
    ...
    if (GetBattlerHoldEffect(battlerId, FALSE) == HOLD_EFFECT_MACHO_BRACE || GetBattlerHoldEffect(battlerId, FALSE) == HOLD_EFFECT_POWER_ITEM)
        ...
        speed *= 2;
	if (holdEffect == HOLD_EFFECT_QUICK_POWDER && (gBattleMons[battlerId].status2 & STATUS2_TRANSFORMED))
		speed = (speed * 150) / 100;
...
s8 getModifiedMovePriority(u16 move, u8 battlerId){
    ...
    bool8 tryToRaisePriority = FALSE;
    bool8 priorityUp = FALSE;
    u8 priorityType = SIGNATURE_PRIORITY_ALWAYS;
    u8 typePriority = 0;
    u16 ability = GetBattlerAbility(battlerId);
    u16 speciesId = GetFormSpeciesId(gBattleMons[battlerId].species, gBattleMons[battlerId].formId);
    u16 currentHP = gBattleMons[battlerId].hp;
    u16 maxHP = gBattleMons[battlerId].maxHP;

    //For Signature Moves
    if(gSignatureMoveList[speciesId].move == move){
        if(gSignatureMoveList[speciesId].modification == SIGNATURE_MOD_PRIORITY){
            priorityType = gSignatureMoveList[speciesId].variable;
            typePriority = gSignatureMoveList[speciesId].chance;
            tryToRaisePriority = TRUE;
        }
        else if (gSignatureMoveList[speciesId].modification2 == SIGNATURE_MOD_PRIORITY){
            priorityType = gSignatureMoveList[speciesId].variable2;
            typePriority = gSignatureMoveList[speciesId].chance2;
            tryToRaisePriority = TRUE;
        }
        else if (gSignatureMoveList[speciesId].modification3 == SIGNATURE_MOD_PRIORITY){
            priorityType = gSignatureMoveList[speciesId].variable3;
            typePriority = gSignatureMoveList[speciesId].chance3;
            tryToRaisePriority = TRUE;
        }
        else if (gSignatureMoveList[speciesId].modification4 == SIGNATURE_MOD_PRIORITY){
            priorityType = gSignatureMoveList[speciesId].variable4;
            typePriority = gSignatureMoveList[speciesId].chance4;
            tryToRaisePriority = TRUE;
        }
        else if (gSignatureMoveList[speciesId].modification5 == SIGNATURE_MOD_PRIORITY){
            priorityType = gSignatureMoveList[speciesId].variable5;
            typePriority = gSignatureMoveList[speciesId].chance5;
            tryToRaisePriority = TRUE;
        }
        else if (gSignatureMoveList[speciesId].modification6 == SIGNATURE_MOD_PRIORITY){
            priorityType = gSignatureMoveList[speciesId].variable6;
    ...
    if(tryToRaisePriority){
        switch(priorityType){
            case SIGNATURE_PRIORITY_ALWAYS:
                priorityUp = TRUE;
            break;
            case SIGNATURE_PRIORITY_IN_CONDITIONS:
                if(typePriority == SIGNATURE_PRIORITY_CONDITION_WEATHER_ANY){
                    if (gBattleWeather & WEATHER_ANY)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_WEATHER_RAIN){
                    if (gBattleWeather & WEATHER_RAIN_ANY)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_WEATHER_SUN){
                    if (gBattleWeather & WEATHER_SUN_ANY)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_WEATHER_SAND){
                    if (gBattleWeather & WEATHER_SANDSTORM_ANY)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_WEATHER_HAIL){
                    if (gBattleWeather & WEATHER_HAIL_ANY)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_TERRAIN_ANY){
                    if (gFieldStatuses & STATUS_TERRAIN_ANY)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_TERRAIN_ELECTRIC){
                    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_TERRAIN_GRASS){
                    if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_TERRAIN_MISTY){
                    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_TERRAIN_PSYCHIC){
                    if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_INVERSE_ROOM){
                    if (gFieldStatuses & STATUS_FIELD_INVERSE_ROOM)
                        priorityUp = TRUE;
                }
                else if(typePriority == SIGNATURE_PRIORITY_CONDITION_TRICK_ROOM){
                    if (gFieldStatuses & STATUS_FIELD_TRICK_ROOM)
                        priorityUp = TRUE;
                }
            break;
            case SIGNATURE_PRIORITY_WHEN_ABOVE_HEALTH:
                if (currentHP >= (maxHP * typePriority) / 100)
                    priorityUp = TRUE;
            break;
            case SIGNATURE_PRIORITY_WHEN_BELOW_HEALTH:
                if (currentHP <= (maxHP * typePriority) / 100)
                    priorityUp = TRUE;
            break;
        }
...
s8 GetMovePriority(u32 battlerId, u16 move)
{
    ...
    priority = getModifiedMovePriority(move, battlerId);
    if (GetBattlerAbility(battlerId) == ABILITY_GALE_WINGS
        && gBattleMoves[move].type == TYPE_FLYING
        && gBattleMons[battlerId].hp >= (gBattleMons[battlerId].maxHP*3)/4)
    {
        priority++;
    }
    else if (GetBattlerAbility(battlerId) == ABILITY_FLAMING_SOUL
	...
	else if (move == MOVE_GRASSY_GLIDE && gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN && IsBattlerGrounded(battlerId))
    {
        priority++;
    }
	...
	if((gStatuses4[battlerId] & STATUS4_COILED_UP) && (gBattleMoves[move].flags & FLAG_STRONG_JAW_BOOST)){
		priority++;
		gStatuses4[battlerId] &= ~(STATUS4_COILED_UP);
	}
    ...
    if (!(gHitMarker & HITMARKER_RUN))
    {
        while (gBattleStruct->mega.battlerId < gBattlersCount)
        {
                ...
                && !(gProtectStructs[gActiveBattler].noValidMoves))
            {
				...
				else if(gBattleMons[gActiveBattler].hp < (gBattleMons[gActiveBattler].maxHP / 2 ))//Only mega evolve while at half HP
					BattleScriptExecute(BattleScript_WildLegendaryMegaEvolution);
...
static void HandleEndTurn_FinishBattle(void)
{
    ...
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
		...
		if (gBattleTypeFlags & BATTLE_TYPE_TRAINER || B_ALWAYS_RESTORE_ITEMS) //Restore Items
...
static void FreeResetData_ReturnToOvOrDoEvolutions(void)
{
    if (!gPaletteFade.active)
    {
        ...
        if (gDexnavBattle && (gBattleOutcome == B_OUTCOME_WON || gBattleOutcome == B_OUTCOME_CAUGHT || gBattleOutcome == B_OUTCOME_MON_FLED))
            IncrementDexNavChain();
		else if (gDexnavBattle && (gBattleOutcome == B_OUTCOME_RAN))
            DecreaseDexNavChain();
        else
            ResetDexNavSearch();
...
static void ReturnFromBattleToOverworld(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER)
    {
        ...
        if(gBattleOutcome == B_OUTCOME_CAUGHT){
            u8 roamerNum = VarGet(VAR_LAST_ROAMER_NUM);
            gSaveBlock2Ptr->roamerFlag[roamerNum] = TRUE;
            ClearRoamerData();
        }
...
u8 getMoveBasePowerFromMon(struct Pokemon *mon, u16 move){
    ...
    return getMoveBasePower(move, speciesId, ability);
}
...
u8 getMoveBasePower(u16 move, u16 speciesId, u16 ability){
    ...
    //Signature Moves
    if(gSignatureMoveList[speciesId].move == move){
        if(gSignatureMoveList[speciesId].modification == SIGNATURE_MOD_POWER)
            movePower = gSignatureMoveList[speciesId].variable;
        else if (gSignatureMoveList[speciesId].modification2 == SIGNATURE_MOD_POWER)
            movePower = gSignatureMoveList[speciesId].variable2;
        else if (gSignatureMoveList[speciesId].modification3 == SIGNATURE_MOD_POWER)
            movePower = gSignatureMoveList[speciesId].variable3;
        else if (gSignatureMoveList[speciesId].modification4 == SIGNATURE_MOD_POWER)
            movePower = gSignatureMoveList[speciesId].variable4;
        else if (gSignatureMoveList[speciesId].modification5 == SIGNATURE_MOD_POWER)
...
u8 getAttackingStat(u16 move, u8 battler){
    ...
    u16 speciesId = GetFormSpeciesId(gBattleMons[battler].species, gBattleMons[battler].formId);
    u16 ability = gBattleMons[battler].ability;
    u16 highestStat = STAT_ATK;

    if(gBattleMons[battler].attack < gBattleMons[battler].spAttack)
        highestStat = STAT_SPATK;

    //Check Move Split
    if(GetBattleMoveSplitFromSpecies(move, speciesId, ability, GetHiddenPowerFromBattler(battler), highestStat) == SPLIT_PHYSICAL)
        stat = STAT_ATK;
    else
        stat = STAT_SPATK;
    ...
    //Signature Moves
    if(gSignatureMoveList[speciesId].move == move){
        if(gSignatureMoveList[speciesId].modification == SIGNATURE_MOD_DAMAGE_STAT)
            stat = gSignatureMoveList[speciesId].variable;
        else if (gSignatureMoveList[speciesId].modification2 == SIGNATURE_MOD_DAMAGE_STAT)
            stat = gSignatureMoveList[speciesId].variable2;
        else if (gSignatureMoveList[speciesId].modification3 == SIGNATURE_MOD_DAMAGE_STAT)
            stat = gSignatureMoveList[speciesId].variable3;
        else if (gSignatureMoveList[speciesId].modification4 == SIGNATURE_MOD_DAMAGE_STAT)
            stat = gSignatureMoveList[speciesId].variable4;
        else if (gSignatureMoveList[speciesId].modification5 == SIGNATURE_MOD_DAMAGE_STAT)
...
u8 getMoveTypeFromMon(struct Pokemon *mon, u16 move){
    ...
    return getMoveType(move, speciesId, ability);
}

u8 getSignatureMoveType(u16 move, u16 speciesId, u16 ability){
    u16 MoveType;
    ...
    //For Signature Moves
    if(gSignatureMoveList[speciesId].move == move){
        if(gSignatureMoveList[speciesId].modification == SIGNATURE_MOD_TYPE)
            MoveType = gSignatureMoveList[speciesId].variable;
        else if (gSignatureMoveList[speciesId].modification2 == SIGNATURE_MOD_TYPE)
            MoveType = gSignatureMoveList[speciesId].variable2;
        else if (gSignatureMoveList[speciesId].modification3 == SIGNATURE_MOD_TYPE)
            MoveType = gSignatureMoveList[speciesId].variable3;
        else if (gSignatureMoveList[speciesId].modification4 == SIGNATURE_MOD_TYPE)
            MoveType = gSignatureMoveList[speciesId].variable4;
        else if (gSignatureMoveList[speciesId].modification5 == SIGNATURE_MOD_TYPE)
    ...
    switch(speciesId){
		...
		case SPECIES_KECLEON:
			if(gSignatureMoveList[speciesId].move == move){
				MoveType = gBattleMons[VarGet(VAR_SIGNATURE_MOVE_TEMP)].type3;

                if(MoveType >= NUMBER_OF_MON_TYPES || VarGet(VAR_SIGNATURE_MOVE_TEMP) == NUMBER_OF_MON_TYPES)
                    MoveType = TYPE_NORMAL;
            }
		break;
    }

    VarSet(VAR_SIGNATURE_MOVE_TEMP, NUMBER_OF_MON_TYPES);
    return MoveType;
}
...
u8 getMoveType(u16 move, u16 speciesId, u16 ability){
    ...
    //Signature Moves
    MoveType = getSignatureMoveType(move, speciesId, ability);
...
u8 getMoveAccuracyFromMon(struct Pokemon *mon, u16 move){
    ...
    return getMoveAccuracy(move, speciesId);
}
...
u8 getMoveAccuracy(u16 move, u16 speciesId){
    ...
    //For Signature Moves
    if(gSignatureMoveList[speciesId].move == move){
        if(gSignatureMoveList[speciesId].modification == SIGNATURE_MOD_ACCURACY)
            Accuracy = gSignatureMoveList[speciesId].variable;
        else if (gSignatureMoveList[speciesId].modification2 == SIGNATURE_MOD_ACCURACY)
            Accuracy = gSignatureMoveList[speciesId].variable2;
        else if (gSignatureMoveList[speciesId].modification3 == SIGNATURE_MOD_ACCURACY)
            Accuracy = gSignatureMoveList[speciesId].variable3;
        else if (gSignatureMoveList[speciesId].modification4 == SIGNATURE_MOD_ACCURACY)
            Accuracy = gSignatureMoveList[speciesId].variable4;
        else if (gSignatureMoveList[speciesId].modification5 == SIGNATURE_MOD_ACCURACY)
...
u8 getMoveSecondaryEffectChance(u16 move, u16 speciesId){
    u8 chances = gBattleMoves[move].secondaryEffectChance;

    //For Signature Moves
    if(gSignatureMoveList[speciesId].move == move){
        if(gSignatureMoveList[speciesId].modification == SIGNATURE_MOD_EFFECT_CHANCE)
            chances = gSignatureMoveList[speciesId].variable;
        else if (gSignatureMoveList[speciesId].modification2 == SIGNATURE_MOD_EFFECT_CHANCE)
            chances = gSignatureMoveList[speciesId].variable2;
        else if (gSignatureMoveList[speciesId].modification3 == SIGNATURE_MOD_EFFECT_CHANCE)
            chances = gSignatureMoveList[speciesId].variable3;
        else if (gSignatureMoveList[speciesId].modification4 == SIGNATURE_MOD_EFFECT_CHANCE)
            chances = gSignatureMoveList[speciesId].variable4;
        else if (gSignatureMoveList[speciesId].modification5 == SIGNATURE_MOD_EFFECT_CHANCE)
...
{
    u32 moveType, ateType, attackerAbility, sigMoveType;
    ...
    attackerAbility = GetBattlerAbility(battlerAtk);
    VarSet(VAR_SIGNATURE_MOVE_TEMP, battlerAtk);
    //GET_MOVE_TYPE(move, moveType);
    //Signature Moves
    moveType = getSignatureMoveType(move, speciesID, attackerAbility);

    if(moveType != gBattleMoves[move].type)
        gBattleStruct->dynamicMoveType = 0x80 | moveType;
